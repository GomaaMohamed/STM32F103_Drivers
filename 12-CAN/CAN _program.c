...
/***************** Enable Clock of CAN Controller ***********************/
	    RCC_voidEnableClock(RCC_APB1,RCC_CAN);
/******************* initialize Bit Timing *****************************/
	      /* You should Be In Initialization Mode Firstly */
	CAN_voidSetOperatingMode(CAN_INITIALIZATION_MODE);
...
/********************* Select RECIEVE FIFO Locked Mode ********************/
	#if       CAN_RECIEVE_FIFO   ==  CAN_RECIEVE_FIFO_UNLOCKED
	       CLR_BIT(CAN_MCR,3);
	#elif    CAN_RECIEVE_FIFO   ==  CAN_RECIEVE_FIFO_LOCKED
	       SET_BIT(CAN_MCR,3);
	#endif
...
void CAN_voidSetOperatingMode(u8 Copy_u8Mode)
{
	...
	{   
       /* from Initialization Mode to Sleep Mode */
		...
		else if (Copy_u8CurrentModeFlag == 2)
		{
		/* Enable Sleep mode */
		 SET_BIT(CAN_MCR,1);
		/* Wait Untile The Initialization Mode to Setup */
	  ...
	  Copy_u8CurrentModeFlag = 0;
	}
/**********************************************************************************************************/
	else if (Copy_u8Mode == CAN_INITIALIZATION_MODE)
	{
		/* from Sleep Mode to Initialization Mode */
        if (Copy_u8CurrentModeFlag == 0){
	    /* Disable Sleep Mode */
		CLR_BIT(CAN_MCR,1);
	...
	Copy_u8CurrentModeFlag = 1;
	}
/**********************************************************************************************************/
	else if (Copy_u8Mode == CAN_NORMAL_MODE)
	{
		...
		while((GET_BIT(CAN_MSR,0) == 1)|(GET_BIT(CAN_MSR,1) == 1)); 
		 ...
		 else if (Copy_u8CurrentModeFlag == 1)
		{
		/* Disable Sleep Mode and init mode */
		CLR_BIT(CAN_MCR,1);
		CLR_BIT(CAN_MCR,0);
...
void CAN_voidSetTestMode(u8 Copy_u8Mode)
{
	/* You should Be In Initialization Mode Firstly */
	 CAN_voidSetOperatingMode(CAN_INITIALIZATION_MODE);
    if (Copy_u8Mode == CAN_SILENT_MODE)
	{
		SET_BIT(CAN_BTR,31);
	}
	else if (Copy_u8Mode == CAN_LOOPBACK_MODE)
	{
		SET_BIT(CAN_BTR,30);
	}
	else if (Copy_u8Mode == CAN_COMBINED_MODE)
	{
		SET_BIT(CAN_BTR,31);
		SET_BIT(CAN_BTR,30);
	}
...
{    
    ...
    /* SET Message Identifier & Standrization & Type Of Data Frame */ 
		    #if     CAN_STANDRIZATION  ==  CAN_STANDRD
		     /* Standard */
		     CLR_BIT(CAN_TI0R,2);
		     /* Id */
		     CAN_TI0R &= ~(0b11111111111 << 21);
		     CAN_TI0R |= (Copy_u32MessageId << 21);
		     /* Data Frame */
		     CLR_BIT(CAN_TI0R,1);
		    #elif  CAN_STANDRIZATION  == CAN_EXTENDED
		     /* Extended */
		     SET_BIT(CAN_TI0R,2);
		     /* Id */
		     CAN_TI0R &= ~(0b11111111111111111111111111111 << 21);
		     CAN_TI0R |= (Copy_u32MessageId << 3);
		     /* Data Frame */
		     CLR_BIT(CAN_TI0R,1);
		    #endif
	
	         /*Set The Length Of Data*/
	        if (Copy_u8DataLength <= 8)
			{
	         CAN_TDT0R &= ~(0b1111 << 0);
		     CAN_TDT0R |= Copy_u8DataLength;}
			   
	       /* Write The Data That Will be Transmitted */
             CAN_TDL0R = Copy_u32DataLow;
		     CAN_TDH0R = Copy_u32DataHigh;
...
/**********************************************************************************************************/
    ...
    /* SET Message Identifier & Standrization & Type Of Data Frame */ 
		   ...
		   {
	    	 CAN_TDT1R &= ~(0b1111 << 0);
		     CAN_TDT1R |= Copy_u8DataLength;}
			   
	        /*Write The Data That Will be Transmitted*/
                   CAN_TDL1R = Copy_u32DataLow;
				   CAN_TDH1R = Copy_u32DataHigh;
	         /*Make a Transmission Request*/
	                  SET_BIT(CAN_TI1R,0);
	          //wait until Transmission become Ok
...
/**********************************************************************************************************/
	else if (GET_BIT(CAN_TSR,28)==1)
	{
        /* SET Message Identifier & Standrization & Type Of Data Frame */ 
		   ...
		   {
	    	   CAN_TDT2R &= ~(0b1111 << 0);
		       CAN_TDT2R |= Copy_u8DataLength;}
			   
	      /*Write The Data That Will be Transmitted*/
                  CAN_TDL2R = Copy_u32DataLow;
				  CAN_TDH2R = Copy_u32DataHigh;
	         /*Make a Transmission Request */
	                  SET_BIT(CAN_TI2R,0);
	         //wait until Transmission become Ok
...
{
	if (Copy_u8TransmissionRequest == CAN_MAILBOX0)
	{
		SET_BIT(CAN_TSR,7);
	}
	else if (Copy_u8TransmissionRequest == CAN_MAILBOX1)
	{
		SET_BIT(CAN_TSR,15);
	}
	else if(Copy_u8TransmissionRequest == CAN_MAILBOX2)
	{
		SET_BIT(CAN_TSR,23);
	}
}
...
{ 
	       Copy_RecievedData->Copy_u8FifoState=0;
	   ...
	   if (Copy_u8FifoNumber == CAN_FIFO0)
	   {
			...
			if (((CAN_RF0R)&(0b11)) > 0)
	        {
				Copy_RecievedData->Copy_u8FifoState=1;
				/* Fill ID of The Recieved Data */
				/* Check Id Type */
				if(GET_BIT(CAN_RI0R,2) == 0)
				{
					Copy_RecievedData->Copy_u32DataId = (CAN_RI0R>>21);
				}
				else if(GET_BIT(CAN_RI0R,2) == 1)
				{
					Copy_RecievedData->Copy_u32DataId = (CAN_RI0R>>3);
				}
				
				/* Fill Frame Type of The Recived Data */
				    Copy_RecievedData->Copy_u32FrameType = GET_BIT(CAN_RI0R,1);
					
				/* Fill Length of The Recived Data */
				    Copy_RecievedData->Copy_u8DataLength = (CAN_RDT0R & 0x01);
					
				/* Fill Index of The Recived Data Flilter */
				    Copy_RecievedData->Copy_u8FilterIndex = (CAN_RDT0R >> 8);
				
				/* Fill  Recived Data LOW */
				    Copy_RecievedData->Copy_u32DataLow = CAN_RDL0R;
					
				/* Fill Fill  Recived Data HIGH */
				    Copy_RecievedData->Copy_u32DataHigh = CAN_RDH0R;
...
/************************************************************************************************************************/
	    else if(Copy_u8FifoNumber == CAN_FIFO1)
	    {
		    ...
		    if (((CAN_RF1R)&(0b11)) > 0)
	        {
				...
				{
					Copy_RecievedData->Copy_u32DataId = (CAN_RI1R>>21);
				}
				else if(GET_BIT(CAN_RI1R,2) == 1)
				{
					Copy_RecievedData->Copy_u32DataId = (CAN_RI1R>>3);
				}
				
				/* Fill Frame Type of The Recived Data */
				    Copy_RecievedData->Copy_u32FrameType = GET_BIT(CAN_RI1R,1);
					
				/* Fill Length of The Recived Data */
				    Copy_RecievedData->Copy_u8DataLength = (CAN_RDT1R & 0x01);
					
				/* Fill Index of The Recived Data Flilter */
				    Copy_RecievedData->Copy_u8FilterIndex = (CAN_RDT1R >> 8);
				
				/* Fill  Recived Data LOW */
				    Copy_RecievedData->Copy_u32DataLow = CAN_RDL1R;
					
				/* Fill Fill  Recived Data HIGH */
				    Copy_RecievedData->Copy_u32DataHigh = CAN_RDH1R;
